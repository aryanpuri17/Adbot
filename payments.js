  ...
  try {
    ...
    for (const event of data.events) {
      ...
      for (const action of event.actions || []) {
        ...
        // Vérifier le memo = ID user EXACT (stricte)
        const comment = (transfer.comment || "").trim();
        const userIdStr = String(userId);
        if (comment !== userIdStr) continue;
  ...
  try {
    ...
    for (const event of data.events) {
      ...
      for (const action of event.actions || []) {
        ...
        const comment = (transfer.comment || "").trim();
        if (comment !== String(userId)) continue;
...
async function checkBSCDeposit(walletAddress, userId, minAmount, symbol, sinceTimestamp) {
  try {
    ...
    for (const tx of data.result) {
      ...
      // Vérifier memo/input = userId EXACT
      let input = tx.input || "";
      try {
        const decoded = Buffer.from(input.replace("0x", ""), "hex").toString("utf8").trim();
        if (decoded !== String(userId)) continue;
      } catch {
        continue;
      }
...
async function verifyDeposit(deposit, config) {
  ...
  const since = Math.floor(new Date(deposit.created_at).getTime() / 1000); // STRICT : pas de marge
