...
bot.on("callback_query", async (q) => {
  ...
  if (data.startsWith("join_ga_")) {
    ...
    const reasons = {
      "not_active": "❌ Concours non actif.",
      "ended": "❌ Concours terminé.",
      "full": "❌ Concours complet.",
      "already_entered": "❌ Déjà inscrit !",
      "insufficient_balance": "❌ Solde insuffisant pour participer."
    };
    return bot.sendMessage(cid, reasons[r?.reason] || "❌ Impossible.");
  }
  ...
  if (data.startsWith("reject_proof_")) {
    ...
    db.db.prepare("UPDATE task_completions SET status = 'rejected' WHERE completion_id = ?").run(cid2);
    return bot.editMessageText(cb_msg(q) + "\n❌ Rejeté", { chat_id: cid, message_id: mid });
  }
  ...
  if (data.startsWith("draw_ga_")) {
    ...
    if (r && r.winners && r.winners.length > 0) {
      r.winners.forEach(winnerId => {
        bot.sendMessage(winnerId,
          `🎉 <b>Tu as gagné le concours !</b>\n💰 +${fmt(r.prizePerWinner)} crédité !`,
          { parse_mode: "HTML" }).catch(() => {});
      });
      return bot.sendMessage(cid, `✅ Tirage effectué ! ${r.winners.length} gagnant(s).`);
    }
    return bot.sendMessage(cid, `⚠️ Aucun participant à ce concours.`);
  }
...
function showFlashTasks(cid, uid, user) {
  // Flash tasks = tâches avec expires_at proche
  let tasks = [];
  try {
    tasks = db.db.prepare(
      "SELECT * FROM tasks WHERE status='active' AND expires_at IS NOT NULL AND expires_at > CURRENT_TIMESTAMP ORDER BY expires_at ASC LIMIT 5"
    ).all();
  } catch (e) { console.error("Flash tasks error:", e.message); }
...
async function doSpin(cid, mid, uid, free, user) {
  ...
  if (!result || result.success === false) {
    if (result && result.reason === "no_free_spins")
      return bot.sendMessage(cid, "❌ Plus de spins gratuits !");
    if (result && result.reason === "insufficient_balance")
...
function showGiveaways(cid) {
  ...
  if (!list.length) txt += "Aucun concours actif. Reviens plus tard !";
    ...
    txt += `🎁 <b>${esc(g.title)}</b>\n💰 ${fmt(g.prize_amount)} | 👥 ${count.n || 0} participants\n⏰ Fin : ${fmtDate(g.ends_at)}\n\n`;
  });
...
function showAdminProofs(cid) {
  const proofs = db.db.prepare(
    "SELECT tc.*, t.title FROM task_completions tc JOIN tasks t ON tc.task_id=t.task_id WHERE tc.status='pending_review' ORDER BY tc.created_at ASC LIMIT 10"
  ).all();
...
bot.on("message", async (msg) => {
  ...
  if (s === "ct_budget") {
    ...
    const platformFee = taskType ? taskType.platform_fee || 0 : 0;
    const maxComp = Math.floor(budget / (data.reward + platformFee));
    if (maxComp < 1) return bot.sendMessage(cid, `❌ Budget trop petit. Min: ${fmt(data.reward + platformFee)}`);
    ...
    if (!taskId) return bot.sendMessage(cid, "❌ Erreur création campagne. Solde insuffisant ?");
    clearState(uid);
    ...
    for (const aid of config.ADMIN_IDS) {
      bot.sendMessage(aid,
        ...
        { parse_mode: "HTML", reply_markup: KBI([[
          { text: `✅ Approuver`, callback_data: `approve_task_${taskId}` },
          { text: `❌ Rejeter`,   callback_data: `reject_task_${taskId}` }
        ]]) }).catch(() => {});
    }
    return;
  }
  ...
  if (s === "flash_url" && isAdmin(uid)) {
    ...
    db.db.prepare(
      `INSERT INTO tasks (creator_id, type, title, link, reward, platform_fee, max_completions, budget, budget_remaining, status, expires_at)
       VALUES (?, 'website', ?, ?, ?, 0, 9999, 9999, 9999, 'active', ?)`
    ).run(uid, data.title, text, data.reward, expiresAt);
  ...
  if (s === "support_msg") {
    clearState(uid);
    db.createTicket(uid, "Support", text, "general");
