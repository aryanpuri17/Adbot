...
bot.on("callback_query", async (q) => {
  ...
  if (data.startsWith("conf_dep_")) {
    ...
    // Récupérer le dépôt sans le confirmer via db.confirmDeposit (qui crédite balance normale)
    ...
    // Marquer confirmé
    db.db.prepare("UPDATE deposits SET status='confirmed', confirmed_at=CURRENT_TIMESTAMP WHERE deposit_id=?").run(depId);
    // Créditer le dépôt non-retirable
    creditDeposit(dep.user_id, dep.amount);
    bot.sendMessage(dep.user_id, `✅ <b>Dépôt confirmé !</b>\n💳 +${fmt(dep.amount)} ajouté à ta balance dépôt !\n\nℹ️ Utilisable pour jeux et campagnes (non retirable).`, { parse_mode: "HTML" }).catch(() => {});
...
payments.startAutoDepositChecker(db, config, async (deposit, usdAmount, tx) => {
  try {
    ...
    if (!user) return;
    // ATTENTION: confirmDeposit dans la DB a déjà crédité balance normale
    // On doit basculer ce montant de balance vers deposit_balance
