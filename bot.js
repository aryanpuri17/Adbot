// ============================================
// 🤖 ADCRYPTON BOT - VERSION PRO v2.1 FIXED
// ============================================
...
// ─────────────────────────────────────────────
//  REPLY KEYBOARDS — Clavier en bas de l'écran
// ─────────────────────────────────────────────

function KB_MAIN(userId) {
  const rows = [
    ["💳 Balance",  "📋 Tâches"],
    ["🎮 Jeux",     "🏆 Concours"],
    ["👥 Parrainage","🎫 Support"],
  ...
  if (isAdmin(userId)) rows.push(["👑 ADMIN"]);
  return {
    keyboard: rows,
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  const requiredChannels = [
    db.getSetting("official_channel", config.CHANNEL_USERNAME),
    db.getSetting("payment_channel", config.PAYMENT_CHANNEL || "")
  ].filter(c => c && c.length > 0);

  if (requiredChannels.length > 0) {
    let allJoined = true;
    for (const ch of requiredChannels) {
      const joined = await checkMembership(ch, userId).catch(() => false);
      if (!joined) { allJoined = false; break; }
    }
    if (!allJoined) {
      const btns = {
        inline_keyboard: [
          ...requiredChannels.map(ch => [{ text: `📢 Rejoindre ${ch}`, url: `https://t.me/${ch.replace("@","")}` }]),
          [{ text: "✅ J'ai rejoint — Continuer", callback_data: "check_channels" }]
        ]
      };
      return bot.sendMessage(chatId,
        `👋 <b>Bienvenue sur ADCRYPTON !</b>\n\n📢 Rejoins nos canaux officiels pour continuer :`,
...
async function sendMainMenu(chatId, user) {
  ...
  const streakTxt = streak >= 3 ? ` | 🔥 ${streak}j streak` : "";
...
✅ Tâches : <b>${user.tasks_completed}</b> | 👥 Filleuls : <b>${user.referral_count}</b>${streakTxt}
📅 Aujourd'hui : <b>${user.daily_tasks_done}/${maxTask}</b>`;

  bot.sendMessage(chatId, text, {
    parse_mode: "HTML",
    reply_markup: KB_MAIN(user.user_id)
  });
}
...
bot.on("callback_query", async (q) => {
  ...
  // Navigation
  if (data === "main" || data === "back_main") { clearState(userId); return sendMainMenu(chatId, user); }

  // Vérification canaux après join
  if (data === "check_channels") {
    ...
    if (!allJoined) {
      return bot.answerCallbackQuery(q.id, { text: "❌ Tu n'as pas encore rejoint tous les canaux !", show_alert: true });
    }
...
function showAdminSettings(chatId, msgId) {
  const settings = [
    ...
    { key: "official_channel",      label: "📢 Canal officiel (@username)" },
    { key: "payment_channel",       label: "💸 Canal paiements (@username)" },
    ...
    { key: "maintenance_mode",      label: "🔧 Mode maintenance (true/false)" },
  ];
  ...
  if (db.payWithdrawal) {
    ...
    if (wd) {
      ...
      bot.sendMessage(wd.user_id,
        ...
        `💵 Montant : <b>${fmt(wd.net_amount)}</b>\n` +
        `👛 Envoyé à : <code>${wd.wallet_address}</code>`,
        { parse_mode: "HTML" }).catch(() => {});

      // Publier sur le canal de paiement
      const payChannel = db.getSetting("payment_channel", config.PAYMENT_CHANNEL || "");
      if (payChannel) {
...
bot.on("message", async (msg) => {
  ...
  // ──── Annuler FSM ────
  if (text === "❌ Annuler" || text === "🏠 Accueil") {
    clearState(userId);
    if (text === "🏠 Accueil") return sendMainMenu(chatId, user);
    return bot.sendMessage(chatId, "❌ Annulé.", { reply_markup: KB_MAIN(userId) });
  }

  // ──── Navigation principale ────
  if (text === "📋 Tâches") {
    const maxT  = db.getSetting("max_tasks_day", 50);
    const flash = db.db ? db.db.prepare("SELECT COUNT(*) as n FROM tasks WHERE is_flash=1 AND flash_expires_at > CURRENT_TIMESTAMP AND status='active'").get() : { n: 0 };
    return bot.sendMessage(chatId,
      `📋 <b>TÂCHES</b>\n\n` +
      `📅 Aujourd'hui : <b>${user.daily_tasks_done}/${maxT}</b>\n` +
      `${flash.n > 0 ? `⚡ <b>${flash.n} Flash Task(s) disponible(s) !</b>\n` : ""}` +
  ...
  if (text === "🏆 Concours") {
    ...
    if (giveaways.length === 0) { txt += "Aucun concours actif."; }
    else { giveaways.forEach(g => { txt += `🎁 <b>${esc(g.title)}</b> — ${fmt(g.prize_amount)}\n   👥 ${g.current_participants} participants | Fin: ${fmtDate(g.ends_at)}\n\n`; }); }
    const btns = { inline_keyboard: [
      ...giveaways.map(g => [{ text: `🎟️ Participer — ${esc(g.title)}`, callback_data: `join_ga_${g.giveaway_id}` }]),
      [{ text: "🏠 Accueil", callback_data: "back_main" }]
    ]};
    return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: btns });
  }

  if (text === "👥 Parrainage") {
    if (!botInfo) botInfo = await bot.getMe();
    const link      = `https://t.me/${botInfo.username}?start=ref_${user.referral_code}`;
    const refBonus  = db.getSetting("referral_bonus", config.REFERRAL_BONUS);
    const refPct    = db.getSetting("referral_percent", config.REFERRAL_PERCENT);
    return bot.sendMessage(chatId,
      `👥 <b>PROGRAMME DE PARRAINAGE</b>\n\n` +
      `🔗 Ton lien :\n<code>${link}</code>\n\n` +
      `💰 Bonus par filleul : <b>${fmt(refBonus)}</b>\n` +
      `📊 Commission sur les gains : <b>${refPct}%</b>\n` +
      `👥 Tes filleuls : <b>${user.referral_count}</b>\n\n` +
      `💡 Partage ton lien et gagne automatiquement !`,
      { parse_mode: "HTML", reply_markup: KB_REFERRAL });
  }

  if (text === "🔗 Mon Lien de Parrainage") {
    if (!botInfo) botInfo = await bot.getMe();
    const link = `https://t.me/${botInfo.username}?start=ref_${user.referral_code}`;
    return bot.sendMessage(chatId, `<code>${link}</code>`, { parse_mode: "HTML" });
  }

  if (text === "🏆 Classement") {
    const top = db.db ? db.db.prepare("SELECT first_name, referral_count FROM users WHERE is_banned=0 ORDER BY referral_count DESC LIMIT 10").all() : [];
    const medals = ["🥇","🥈","🥉"];
    let txt = `🏆 <b>CLASSEMENT</b>\n\n`;
    top.forEach((u, i) => { txt += `${medals[i] || `${i+1}.`} <b>${esc(u.first_name)}</b> — ${u.referral_count} filleuls\n`; });
    return bot.sendMessage(chatId, txt || "Aucun résultat.", { parse_mode: "HTML", reply_markup: KB_REFERRAL });
  }

  if (text === "➕ Créer une Campagne") {
    const types = Object.entries(config.TASK_TYPES).filter(([,v]) => v.enabled);
    const btns  = { inline_keyboard: [
      ...types.map(([key, t]) => [{ text: t.name, callback_data: `create_task_type_${key}` }]),
  ...
  if (text === "🎁 Bonus") {
    ...
    if (!result) {
      const next = new Date(user.last_bonus_at);
      next.setHours(next.getHours() + 24);
      return bot.sendMessage(chatId, `🎁 Déjà réclamé !\nProchain bonus : <b>${fmtDate(next)}</b>`, { parse_mode: "HTML" });
    }
    const streak = (user.streak_days || 0) + 1;
    db.updateUser(userId, { streak_days: streak });
    const streakEmoji = streak >= 7 ? "🔥🔥🔥" : streak >= 3 ? "⚡⚡" : "📅";
    return bot.sendMessage(chatId,
      `🎁 <b>BONUS RÉCLAMÉ !</b>\n\n💰 +${fmt(result.amount)}\n${streakEmoji} Streak : <b>${streak}j</b>${streak >= 3 ? ` (bonus x${streak >= 7 ? "2" : "1.5"})` : ""}\n\n💵 Nouveau solde : ${fmt((db.getUser(userId) || user).balance)}`,
      { parse_mode: "HTML" });
  }
  ...
  // ──── Navigation Jeux ────
  if (text === "🎡 Roue de Fortune") {
    const cost = parseFloat(db.getSetting("spin_cost", config.SPIN_WHEEL.cost));
    const prizes = config.SPIN_WHEEL.prizes;
    const btns = { inline_keyboard: [
      ...(user.free_spins > 0 ? [[{ text: `🎟️ Spin Gratuit (${user.free_spins})`, callback_data: "spin_free" }]] : []),
      ...(user.balance >= cost ? [[{ text: `🎡 Spin — ${fmt(cost)}`, callback_data: "spin_paid" }]] : []),
      [{ text: "◀️ Jeux", callback_data: "back_main" }]
    ]};
    return bot.sendMessage(chatId,
      `🎡 <b>ROUE DE FORTUNE</b>\n\n${prizes.map(p => `${p.label} — ${p.chance}%`).join("\n")}\n\n💵 Solde : ${fmt(user.balance)}`,
      { parse_mode: "HTML", reply_markup: btns });
  }
  ...
  if (text === "🎲 Dés") {
    ...
    const bets   = [0.05, 0.10, 0.25, 0.50, 1.00, 2.00, 5.00].filter(b => b >= minBet && b <= maxBet && b <= user.balance);
    const rows   = [];
    for (let i = 0; i < bets.length; i += 3) rows.push(bets.slice(i,i+3).map(b => ({ text: fmt(b), callback_data: `dice_bet_${b}` })));
  ...
  if (text === "🏆 Jackpot") {
    ...
    const chance  = parseFloat(db.getSetting("jackpot_chance", "5"));
    return bot.sendMessage(chatId,
      `🏆 <b>JACKPOT PROGRESSIF</b>\n\n💰 Pool : <b>${fmt(jackpot)}</b>\n🎟️ Ticket : ${fmt(cost)}\n🎯 Chance : ${chance}%\n\n💵 Solde : ${fmt(user.balance)}`,
      { parse_mode: "HTML", reply_markup: { inline_keyboard: [
        user.balance >= cost ? [{ text: `🎟️ Jouer — ${fmt(cost)}`, callback_data: "jackpot_play" }] : [{ text: "❌ Solde insuffisant", callback_data: "jackpot" }],
        [{ text: "◀️ Retour", callback_data: "back_main" }]
      ]}});
  }
  ...
  if (text === "💳 Déposer") {
    const methods = config.DEPOSIT_METHODS;
    const btns = Object.entries(methods).filter(([,m]) => m.enabled)
      .map(([key, m]) => [{ text: `${m.name} (min ${m.minAmount} ${m.symbol})`, callback_data: `dep_method_${key}` }]);
    btns.push([{ text: "🏠 Accueil", callback_data: "back_main" }]);
    return bot.sendMessage(chatId,
      `💳 <b>DÉPOSER</b>\n\n⚡ Prix temps réel — Confirmation automatique\n\nChoisis ta crypto :`,
  ...
  if (text === "🏧 Retirer") {
    ...
    const feeP = parseFloat(db.getSetting("withdrawal_fee_percent", config.WITHDRAWAL_FEE_PERCENT));
    const btns = Object.entries(config.WITHDRAWAL_METHODS).filter(([,m]) => m.enabled)
      .map(([key, m]) => [{ text: m.name, callback_data: `wd_method_${key}` }]);
    btns.push([{ text: "🏠 Accueil", callback_data: "back_main" }]);
    return bot.sendMessage(chatId,
      `🏧 <b>RETRAIT</b>\n\n💵 Solde : ${fmt(user.balance)}\n📌 Min : ${fmt(minW)}\n💸 Frais : ${feeP}%\n\nChoisis la méthode :`,
      { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
  ...
  if (text === "📋 Historique") {
    const deps = db.getUserDeposits(userId, 5);
    let txt = `📋 <b>HISTORIQUE</b>\n\n`;
    if (deps.length === 0) { txt += "Aucun dépôt."; }
    else { deps.forEach(d => { txt += `• ${d.amount}$ — ${d.status} — ${fmtDate(d.created_at)}\n`; }); }
    return bot.sendMessage(chatId, txt, { parse_mode: "HTML" });
  }

  // ──── TÂCHES par catégorie ────
  const taskTypes = { "📢 Canaux": "channel", "👥 Groupes": "group", "🤖 Bots": "bot", "📺 YouTube": "youtube", "🐦 Twitter": "twitter", "📷 Instagram": "instagram", "🎵 TikTok": "tiktok", "🌐 Sites Web": "website", "📱 Apps": "app" };
  if (taskTypes[text]) {
    const type  = taskTypes[text];
    const tasks = db.getAvailableTasks(userId, 5, type);
    if (!tasks || tasks.length === 0) {
      return bot.sendMessage(chatId, `📋 Aucune tâche disponible en ce moment.\nReviens plus tard !`);
    }
    for (const task of tasks) {
      const btns = { inline_keyboard: [
        ...(task.url ? [[{ text: "🔗 Ouvrir le lien", url: task.url }]] : []),
        [{ text: "✅ Valider ma participation", callback_data: `verify_task_${task.task_id}` }]
      ]};
      await bot.sendMessage(chatId,
        `📋 <b>${esc(task.title)}</b>\n\n💰 Récompense : <b>${fmt(task.reward_per_completion)}</b>\n${task.description ? esc(task.description) + "\n" : ""}\n1. Ouvre le lien\n2. Effectue l'action\n3. Valide`,
        { parse_mode: "HTML", reply_markup: btns });
    }
    return;
  }
  ...
  if (text === "⚡ Flash Tasks") {
    const flash = db.db ? db.db.prepare("SELECT * FROM tasks WHERE is_flash=1 AND flash_expires_at > CURRENT_TIMESTAMP AND status='active'").all() : [];
    if (flash.length === 0) return bot.sendMessage(chatId, "⚡ Aucune Flash Task active. Reviens plus tard !");
    const btns = { inline_keyboard: [
      ...flash.map(t => [{ text: `⚡ ${esc(t.title)} — ${fmt(t.reward_per_completion * 2)}`, callback_data: `verify_task_${t.task_id}` }])
    ]};
    return bot.sendMessage(chatId, `⚡ <b>FLASH TASKS</b>\n\n🔥 Récompense x2 !\n⏰ Durée limitée !`, { parse_mode: "HTML", reply_markup: btns });
  }

  // ──── ADMIN via reply keyboard ────
  if (!isAdmin(userId)) {
    // Gérer FSM si état actif
    return handleFSM(msg, userId, chatId, text, state, user);
  }

  if (text === "👑 ADMIN") return sendAdminMenu(chatId);
  if (text === "📊 Stats")         return sendAdminStats(chatId);
  if (text === "⚙️ Config Bot")    return sendAdminSettings(chatId, userId);
  if (text === "🎮 Config Jeux")   return sendAdminGamesSettings(chatId, userId);
  if (text === "⚡ Flash Tasks")   {
    return bot.sendMessage(chatId,
      `⚡ <b>FLASH TASKS ADMIN</b>\n\nTâches x2 pendant 30 minutes.`,
      { parse_mode: "HTML", reply_markup: { inline_keyboard: [
        [{ text: "➕ Créer Flash Task", callback_data: "admin_new_flash" }]
      ]}});
  }
  // tasks handled by general tasks handler above
  if (text === "📸 Preuves")       return sendAdminPanel(chatId, "proofs");
  if (text === "🏧 Retraits")      return sendAdminPanel(chatId, "withdrawals");
  if (text === "💳 Dépôts")        return sendAdminPanel(chatId, "deposits");
  if (text === "👥 Users")         return sendAdminPanel(chatId, "users");
  if (text === "🎫 Tickets")       return sendAdminPanel(chatId, "tickets");
  if (text === "🏆 Concours")      return sendAdminPanel(chatId, "giveaways");
  if (text === "📢 Broadcast") {
    setState(userId, "broadcast");
    return bot.sendMessage(chatId, "📢 Envoie ton message broadcast :", { reply_markup: KB_CANCEL });
  }

  if (text === "⛔ Bannir") {
    setState(userId, "adm_ban_uid");
    return bot.sendMessage(chatId, "⛔ Envoie l'ID à bannir :", { reply_markup: KB_CANCEL });
  }
...
// Fonctions admin via reply keyboard
...
function sendAdminSettings(chatId, userId) {
  const settings = [
    { key: "bot_name", label: "🤖 Nom du bot" },
  ...
  const btns = { inline_keyboard: settings.map(s => [{ text: `✏️ ${s.label}`, callback_data: `adm_set_${s.key}` }]) };
  return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: btns });
}

function sendAdminGamesSettings(chatId, userId) {
  const settings = [
    { key: "spin_cost", label: "🎡 Coût spin" },
    { key: "dice_min_bet", label: "🎲 Dés min" },
    { key: "dice_max_bet", label: "🎲 Dés max" },
    { key: "dice_multiplier", label: "🎲 Dés mult" },
    { key: "coinflip_min_bet", label: "🪙 PF min" },
    { key: "coinflip_max_bet", label: "🪙 PF max" },
    { key: "coinflip_multiplier", label: "🪙 PF mult" },
...
function sendAdminPanel(chatId, section) {
  if (section === "tasks") return showAdminTasksMsg(chatId);
  if (section === "proofs") return showAdminProofsMsg(chatId);
  if (section === "withdrawals") return showAdminWithdrawalsMsg(chatId);
  if (section === "deposits") return showAdminDepositsMsg(chatId);
  if (section === "users") return bot.sendMessage(chatId, "👥 Envoie l'ID user pour le gérer :", { reply_markup: { inline_keyboard: [[{ text: "💰 Modifier Solde", callback_data: "admin_add_balance" }, { text: "⛔ Bannir", callback_data: "admin_ban" }]] }});
  if (section === "tickets") return showAdminTicketsMsg(chatId);
  if (section === "giveaways") return showAdminGiveawaysMsg(chatId);
}

function showAdminTasksMsg(chatId) {
  const tasks = db.getPendingTasks();
  if (tasks.length === 0) return bot.sendMessage(chatId, "✅ Aucune tâche en attente.");
  let txt = `📋 <b>TÂCHES (${tasks.length})</b>\n\n`;
  const btns = [];
  tasks.slice(0,5).forEach(t => {
    txt += `#${t.task_id} ${esc(t.title)} — ${fmt(t.reward_per_completion)}\n`;
    btns.push([{ text: `✅ #${t.task_id}`, callback_data: `adm_approve_task_${t.task_id}` }, { text: `❌`, callback_data: `adm_reject_task_${t.task_id}` }]);
  });
  return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
}

function showAdminProofsMsg(chatId) {
  const proofs = db.getPendingProofs ? db.getPendingProofs() : [];
  if (proofs.length === 0) return bot.sendMessage(chatId, "✅ Aucune preuve en attente.");
  const btns = proofs.slice(0,5).map(p => [{ text: `✅ #${p.completion_id}`, callback_data: `adm_approve_proof_${p.completion_id}` }, { text: `❌`, callback_data: `adm_reject_proof_${p.completion_id}` }]);
  return bot.sendMessage(chatId, `📸 <b>PREUVES (${proofs.length})</b>`, { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
}

function showAdminWithdrawalsMsg(chatId) {
  const wds = db.getPendingWithdrawals();
  if (wds.length === 0) return bot.sendMessage(chatId, "✅ Aucun retrait en attente.");
  let txt = `🏧 <b>RETRAITS EN ATTENTE (${wds.length})</b>\n\n`;
  const btns = [];
  wds.slice(0,5).forEach(w => {
    txt += `#${w.withdrawal_id} | ${esc(w.first_name)}\n`;
    txt += `💵 ${fmt(w.net_amount)} → <code>${w.wallet_address || "?"}</code>\n\n`;
    btns.push([
      { text: `✅ Confirmé #${w.withdrawal_id}`, callback_data: `adm_pay_wd_${w.withdrawal_id}` },
      { text: `❌ Rejeter`, callback_data: `adm_reject_wd_${w.withdrawal_id}` }
    ]);
  });
  return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
}

function showAdminDepositsMsg(chatId) {
  const deps = db.getPendingDeposits ? db.getPendingDeposits() : [];
  if (deps.length === 0) return bot.sendMessage(chatId, "✅ Aucun dépôt en attente.");
  let txt = `💳 <b>DÉPÔTS (${deps.length})</b>\n\n`;
  const btns = [];
  deps.forEach(d => {
    txt += `#${d.deposit_id} ${esc(d.first_name)} — ${d.amount}$\n`;
    btns.push([{ text: `✅ #${d.deposit_id}`, callback_data: `adm_confirm_dep_${d.deposit_id}` }, { text: `❌`, callback_data: `adm_reject_dep_${d.deposit_id}` }]);
  });
  return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
}

function showAdminTicketsMsg(chatId) {
  const tickets = db.getOpenTickets ? db.getOpenTickets() : [];
  if (tickets.length === 0) return bot.sendMessage(chatId, "✅ Aucun ticket ouvert.");
  const btns = tickets.slice(0,5).map(t => [{ text: `🎫 #${t.ticket_id} — ${esc(t.subject)}`, callback_data: `adm_ticket_${t.ticket_id}` }]);
  return bot.sendMessage(chatId, `🎫 <b>TICKETS (${tickets.length})</b>`, { parse_mode: "HTML", reply_markup: { inline_keyboard: btns } });
}

function showAdminGiveawaysMsg(chatId) {
  const ga = db.getActiveGiveaways ? db.getActiveGiveaways() : [];
  let txt = `🏆 <b>CONCOURS</b>\n\n`;
  ga.forEach(g => { txt += `#${g.giveaway_id} ${esc(g.title)} — ${fmt(g.prize_amount)}\n`; });
  if (ga.length === 0) txt += "Aucun concours actif.";
  const btns = { inline_keyboard: [
    [{ text: "➕ Nouveau Concours", callback_data: "admin_new_giveaway" }],
    ...ga.map(g => [{ text: `🎲 Tirer #${g.giveaway_id}`, callback_data: `adm_draw_ga_${g.giveaway_id}` }])
  ]};
  return bot.sendMessage(chatId, txt, { parse_mode: "HTML", reply_markup: btns });
}

// ─────────────────────────────────────────────
//  FSM HANDLER (états de conversation)
// ─────────────────────────────────────────────

async function handleFSM(msg, userId, chatId, text, state, user) {
  if (!state) return;
  const s    = state.state;
