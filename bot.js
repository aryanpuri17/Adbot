...
async function handleStartTask(cid, uid, taskId, user) {
  ...
  if (task.type === "bot" || task.type === "miniapp") {
    // Démarrer la complétion si pas déjà fait
    if (!existing) {
      try {
        const r = db.startTaskCompletion(taskId, uid);
        if (!r) {
          // Création manuelle de l'entrée si la fonction DB échoue
          db.db.prepare(
            "INSERT INTO task_completions (task_id, user_id, status, completed_at) VALUES (?, ?, 'pending', CURRENT_TIMESTAMP)"
          ).run(taskId, uid);
        }
      } catch (e) {
        console.error("startTask error:", e.message);
        // Tenter quand même
        try {
          db.db.prepare(
            "INSERT INTO task_completions (task_id, user_id, status, completed_at) VALUES (?, ?, 'pending', CURRENT_TIMESTAMP)"
