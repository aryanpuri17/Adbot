...
${user.balance >= budget ? "✅ Prêt à créer !" : "⚠️ Solde insuffisant !"}`;
  ...
  if (state.state === "deposit_amount") {
    ...
    const msg2 = await payments.buildDepositMessage(state.data.method, amount, method.wallet, userId, depId);
...
payments.startAutoDepositChecker(db, config, async (deposit, usdAmount, tx) => {
  try {
    ...
    const cryptoSymbols = { ton: "TON", bnb: "BNB", usdt_bep20: "USDT", usdt_ton: "USDT" };
    const symbol = cryptoSymbols[deposit.method] || "CRYPTO";

    await bot.sendMessage(deposit.user_id,
      `✅ <b>Dépôt confirmé automatiquement !</b>\n\n` +
      `💰 Montant : <b>${tx.amount} ${symbol}</b>\n` +
      `💵 Crédité : <b>${usdAmount}$</b>\n` +
      `🔗 TX : <code>${tx.txHash}</code>\n\n` +
      `💳 Nouveau solde : <b>${(user.balance + usdAmount).toFixed(2)}$</b>`,
      { parse_mode: "HTML" }
    );

    console.log(`✅ Dépôt auto confirmé: User ${deposit.user_id} +${usdAmount}$`);
  } catch (e) {
    console.error("Notification dépôt auto error:", e.message);
  }
});
