...
function fmt(amountUSD) {
  ...
  let val = amountUSD;
  let decimals = 2;
  if (cur === "TON") { val = amountUSD / conversionCache.ton; decimals = 4; }
  else if (cur === "BNB") { val = amountUSD / conversionCache.bnb; decimals = 5; }
  return `${Number(val || 0).toFixed(decimals)}${currencySymbol(cur)}`;
}
...
const KB_TASKS    = KB([["📢 Canaux","👥 Groupes"],["🤖 Bots","📺 YouTube"],["🐦 Twitter","📷 Instagram"],["🎵 TikTok","🌐 Sites Web"],["📱 Apps"],["➕ Créer Campagne","🎁 Bonus Quotidien"],["🏠 Accueil"]]);
...
bot.on("callback_query", async (q) => {
  ...
  if (data.startsWith("ct_type_")) {
    ...
    return bot.sendMessage(cid, `➕ <b>Titre de ta campagne :</b>`, { parse_mode: "HTML", reply_markup: KB_CANCEL });
  }
  if (data.startsWith("ct_proof_")) {
    const choice = data.replace("ct_proof_","");
    const st = getState(uid);
    if (!st) return;
    st.data.proofRequired = (choice === "yes");
    setState(uid, "ct_link", st.data);
    return bot.sendMessage(cid, `🔗 <b>URL de la campagne :</b>\n\nExemple : https://t.me/MonCanal`, { parse_mode: "HTML", reply_markup: KB_CANCEL });
  }
  ...
  if (data.startsWith("conf_dep_")) {
    ...
    if (dep) bot.sendMessage(dep.user_id, `✅ <b>Dépôt confirmé !</b>\n💰 +${fmt(dep.amount)} crédité !`, { parse_mode: "HTML" }).catch(() => {});
...
async function showTasksByType(cid, uid, type, user) {
  ...
  if (!tasks || tasks.length === 0) {
    return bot.sendMessage(cid, `📋 Aucune tâche disponible dans cette catégorie.\nReviens plus tard !`);
  }

  const typeNames = {
    channel: "📢 Canal", group: "👥 Groupe", bot: "🤖 Bot",
    youtube: "📺 YouTube", twitter: "🐦 Twitter", instagram: "📷 Instagram",
    tiktok: "🎵 TikTok", website: "🌐 Site", app: "📱 App"
  };

  for (const task of tasks.slice(0, 3)) {
    const tt   = config.TASK_TYPES[task.type] || {};
    const dur  = tt.min_stay_hours
      ? `${tt.min_stay_hours}h`
      : tt.min_stay_seconds
      ? `${tt.min_stay_seconds}s`
      : task.proof_required ? "Preuve requise" : "Instantané";

    const text =
      `┌─────────────────────┐\n` +
      `  ${typeNames[task.type] || "📋"} <b>${esc(task.title)}</b>\n` +
      `└─────────────────────┘\n\n` +
      `💰 Récompense : <b>${fmt(task.reward)}</b>\n` +
      `⏱ Durée minimum : <b>${dur}</b>\n` +
      (task.description ? `\n📝 ${esc(task.description)}\n` : "");

    await bot.sendMessage(cid, text, {
      parse_mode: "HTML",
      reply_markup: KBI([[{ text: "▶️ Démarrer la tâche", callback_data: `start_task_${task.task_id}` }]])
    });
  }
}
...
async function handleStartTask(cid, uid, taskId, user) {
  ...
  if (existing) {
    if (existing.status === "verified") return bot.sendMessage(cid, "✅ Tu as déjà complété cette tâche.");
    if (existing.status === "pending_review") return bot.sendMessage(cid, "⏳ Ta preuve est en cours de vérification.");
  }

  // Si déjà commencée, juste vérifier
  ...
  if (!r) return bot.sendMessage(cid, "❌ Erreur démarrage.");

  const tt = config.TASK_TYPES[task.type] || {};

  // Pour canaux/groupes — vérifier abonnement immédiatement
  if ((task.type === "channel" || task.type === "group") && task.chat_id) {
    const ok = await isMember(task.chat_id, uid);
    if (!ok) {
      return bot.sendMessage(cid,
        `❌ Tu n'es pas encore abonné !\n\n1️⃣ Rejoins le canal\n2️⃣ Reviens et clique "✅ J'ai rejoint"`,
        { reply_markup: KBI([
          [{ text: "🔗 Rejoindre", url: task.link }],
          [{ text: "✅ J'ai rejoint", callback_data: `verify_task_${taskId}` }]
        ]) });
    }

    // Déjà abonné — message d'attente + timer
    if (tt.min_stay_hours) {
      const hours = tt.min_stay_hours;
      return bot.sendMessage(cid,
        `✅ <b>Abonnement vérifié !</b>\n\n` +
        `📌 ${esc(task.title)}\n` +
        `💰 Récompense : <b>${fmt(task.reward)}</b>\n\n` +
        `⏳ <b>Attends ${hours}h</b> pour recevoir ta récompense.\n` +
        `⚠️ <b>Ne te désabonne pas avant !</b>\n\n` +
        `Tu seras notifié automatiquement.`,
        { parse_mode: "HTML" });
    }

    // Pas de min_stay → valider direct
    return handleVerifyTask(cid, uid, taskId, user);
  }
  ...
  if (task.type === "bot") {
    return bot.sendMessage(cid,
      ...
      `1️⃣ Clique le lien et lance le bot\n` +
      `2️⃣ Reviens dans ${tt.min_stay_seconds || 30}s\n` +
      `3️⃣ Clique "Valider"`,
      { parse_mode: "HTML", reply_markup: KBI([
        [{ text: "🔗 Ouvrir le bot", url: task.link }],
        [{ text: `✅ Valider`, callback_data: `verify_task_${taskId}` }]
      ]) });
  }

  // Tâches avec preuve (YouTube, Twitter, etc.)
  if (task.proof_required) {
    return bot.sendMessage(cid,
      `▶️ <b>Tâche démarrée !</b>\n\n` +
      `1️⃣ Ouvre le lien et effectue l'action\n` +
      `2️⃣ Fais une capture d'écran\n` +
      `3️⃣ Clique "Envoyer la preuve"`,
      { parse_mode: "HTML", reply_markup: KBI([
        [{ text: "🔗 Ouvrir", url: task.link }],
        [{ text: "📸 Envoyer la preuve", callback_data: `send_proof_${taskId}` }]
      ]) });
  }

  // Tâches simples (website, etc.)
  return bot.sendMessage(cid,
...
async function handleVerifyTask(cid, uid, taskId, user) {
  ...
  // Vérif abonnement pour canal/groupe
  if ((task.type === "channel" || task.type === "group") && task.chat_id) {
    const ok = await isMember(task.chat_id, uid);
    if (!ok) return bot.sendMessage(cid, "❌ Tu n'es pas/plus abonné !",
      { reply_markup: KBI([[{ text: "🔗 Rejoindre", url: task.link }]]) });
  }

  const r = db.verifyTaskCompletion(taskId, uid);
  if (r.success) {
    return bot.sendMessage(cid, `✅ <b>Tâche validée !</b>\n💰 +${fmt(r.reward)} crédité !`, { parse_mode: "HTML" });
  }
  if (r.reason === "too_early") {
    return bot.sendMessage(cid, `⏳ Attends encore <b>${fmtDuration(r.remaining)}</b> !`, { parse_mode: "HTML" });
  }
  if (r.reason === "not_found") {
    return bot.sendMessage(cid, "❌ Démarre d'abord la tâche.");
  }
...
async function doSpin(cid, mid, uid, free, user) {
  ...
  if (!result || result.success === false) {
    if (result?.reason === "no_free_spins") return bot.sendMessage(cid, "❌ Plus de spins gratuits !");
    if (result?.reason === "insufficient_balance") return bot.sendMessage(cid, `❌ Solde insuffisant. Besoin : ${fmt(cost)}`);
    return bot.sendMessage(cid, "❌ Erreur spin.");
  }
...
async function playDice(cid, uid, bet, user) {
  ...
  db.updateBalance(uid, -bet, "dice_bet", `Dés mise ${fmtUSD(bet)}`);
  ...
  if (val >= 4) {
    ...
    db.updateBalance(uid, win, "dice_win", `Dés gain ${fmtUSD(win)}`);
  } else {
...
async function playCoinflip(cid, uid, choice, bet, user) {
  ...
  db.updateBalance(uid, -bet, "cf_bet", `Pile/Face mise ${fmtUSD(bet)}`);
  ...
  if (won) {
    win = Math.round(bet * mult * 100) / 100;
    db.updateBalance(uid, win, "cf_win", `Pile/Face gain ${fmtUSD(win)}`);
  } else {
...
async function playJackpot(cid, uid, user) {
  ...
  db.updateBalance(uid, -cost, "jackpot_bet", `Jackpot ticket`);
  const newPool = Math.round((pool + cost * 0.50) * 100) / 100;
  const isWin = pool > 0 && Math.random() * 100 < chance;

  await new Promise(r => setTimeout(r, 2000));
  if (isWin) {
    db.updateBalance(uid, pool, "jackpot_win", `Jackpot gagné`);
    db.setSetting("jackpot_pool","0");
    for (const aid of config.ADMIN_IDS) bot.sendMessage(aid, `🏆 Jackpot gagné par ${uid} : ${fmt(pool)}`).catch(() => {});
    bot.sendMessage(cid, `🎉🎉 <b>JACKPOT !</b> 🎉🎉\n\nTu as gagné <b>${fmt(pool)}</b> !\n💵 Balance : ${fmt((db.getUser(uid)).balance)}`, { parse_mode: "HTML" });
  } else {
...
async function playGuess(cid, uid, guess, user) {
  ...
  db.updateBalance(uid, -cost, "guess_bet", `Devinette`);
  const secret = Math.floor(Math.random() * (range[1] - range[0] + 1)) + range[0];

  let win = 0;
  if (guess === secret) {
    win = prize;
    db.updateBalance(uid, win, "guess_win", `Devinette gain`);
  } else {
...
bot.on("message", async (msg) => {
  ...
  // ─── Tâches ───
  const typeMap = {
    "📢 Canaux":"channel", "👥 Groupes":"group", "🤖 Bots":"bot",
    "📺 YouTube":"youtube", "🐦 Twitter":"twitter", "📷 Instagram":"instagram",
    "🎵 TikTok":"tiktok", "🌐 Sites Web":"website", "📱 Apps":"app",
  };
  ...
  if (isAdmin(uid)) {
    ...
    if (text === "💱 Devises")        return showConfigCurrency(cid);
  ...
  // Créer campagne — FSM
  if (s === "ct_title") {
    setState(uid, "ct_proof_q", { ...data, title: text });
    return bot.sendMessage(cid, "📸 <b>Preuve photo requise ?</b>", {
      parse_mode: "HTML",
      reply_markup: KBI([[
        { text: "✅ Oui", callback_data: "ct_proof_yes" },
        { text: "❌ Non", callback_data: "ct_proof_no" }
      ]])
    });
  }
  if (s === "ct_link") {
    setState(uid, "ct_reward", { ...data, link: text });
  ...
  if (s === "ct_reward") {
    ...
    setState(uid, "ct_completions", { ...data, reward });
    return bot.sendMessage(cid, `🎯 Nombre de complétions max ?`, { reply_markup: KB_CANCEL });
  }
  if (s === "ct_completions") {
    const maxC = parseInt(text);
    if (isNaN(maxC) || maxC < 1) return bot.sendMessage(cid, "❌ Invalide.");
    const tt  = config.TASK_TYPES[data.type] || {};
    const fee = tt.platform_fee || 0;
    const budget = (data.reward + fee) * maxC;
    user = db.getUser(uid);
    if (user.balance < budget) return bot.sendMessage(cid, `❌ Solde insuffisant. Besoin : ${fmt(budget)} | Balance : ${fmt(user.balance)}`);
    ...
    bot.sendMessage(cid, `✅ <b>Campagne soumise !</b>\n\n📌 ${esc(data.title)}\n💰 ${fmt(data.reward)}/complétion\n🎯 ${maxC} complétions\n💵 Budget : ${fmt(budget)}\n\nEn attente de validation.`, { parse_mode: "HTML", reply_markup: KB_MAIN(uid) });
    for (const aid of config.ADMIN_IDS) {
      bot.sendMessage(aid,
        `📋 <b>Nouvelle campagne #${taskId}</b>\n👤 ${esc(user.first_name)}\n📌 ${esc(data.title)}\n💰 ${fmt(data.reward)} × ${maxC}\n🔗 ${esc(data.link)}`,
        { parse_mode: "HTML", reply_markup: KBI([[
